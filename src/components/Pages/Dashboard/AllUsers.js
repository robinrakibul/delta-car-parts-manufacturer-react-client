...
const AllUsers = () => {
    const { data: users, isLoading, refetch } = useQuery('users', () => fetch('https://manufacturer-node-server.onrender.com/users').then(res => res.json()));
    if (isLoading) {
    ...
    return (
        <div>
            <h2 className='mt-5 font-bold text-3xl mb-2'>All Users :{users.length}</h2>
